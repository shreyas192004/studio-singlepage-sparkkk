...
const Index = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const { data, error } = await supabase.from("products").select("*").eq("visibility", "public");
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-12 text-white">
            ...
            <p className="max-w-2xl mx-auto text-lg text-primary-foreground/60 mt-6">
              Transform your ideas into unique custom apparel using cutting-edge artificial intelligence. No design
              skills required.
            </p>
          </div>
          ...
          <div className="flex gap-4 justify-center">
            <Link to="/ai-generator">
              <Button
                size="lg"
                className="bg-accent hover:bg-accent/90 text-accent-foreground font-bold py-3 px-6 rounded-lg text-lg"
              >
            ...
            {!user && (
              <Link to="/auth">
                <Button
                  size="lg"
                  variant="outline"
                  className="bg-transparent border-2 border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10 font-bold py-3 px-6 rounded-lg text-lg"
                >
                  Create Account
                </Button>
              </Link>
      ...
      <section className="relative h-[500px] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-primary/60 flex items-center justify-center">
          <div className="text-center text-primary-foreground">
            ...
            <Button
              size="lg"
              className="bg-accent text-accent-foreground hover:bg-accent/90 px-8 py-6 text-lg rounded-lg"
            >
              Shop Sale
            </Button>
          </div>
      ...
      <footer className="bg-primary text-primary-foreground">
        ...
        <div className="border-t border-primary-foreground/10">
          <div className="container mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-primary-foreground/60">
            ...
            <p>
              {" "}
              © 2035 by TESORA. Created with <a href="www.bisugentech.in">BISUGENTECH.IN</a>
            </p>
          </div>
        </div>
      </footer>
